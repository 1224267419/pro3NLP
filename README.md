...
- **MRPC 数据集(判断句子对是否有相同的含义)**
  - 任务类型: 句子对的二分类
  - 评估指标: ACC   F1
- STS-B 数据集(判断句子对的**相似程度**)  [0, 5]  值越高相似度越大
  - 任务类型: 多分类/ 回归
  - 评估指标: Pearson-Spearman Corr
- QQP 数据集
  - 任务类型: 二分类
  - 评估指标: ACC F1
- MNLI 数据集 :判断句子是否蕴含或矛盾
  - 任务类型: 多分类(3)
  - 评估指标: ACC
- QNLI 数据集/RTE 数据集/WNLI 数据集
 ...
 使用:[download_glue_data.py](4_fasttext\download_glue_data.py)脚本下载上述数据集 
...
由于网络问题,无法使用代码自带的下载器,因此可以本地下载文件后再加载模型,[方法](https://blog.csdn.net/weixin_45651616/article/details/130682072),通过下载本地模型再运

行

huggingface研究机构向我们提供了针对GLUE数据集合任务类型的微调脚本, 这些**微调脚本的核⼼**都是**微调**模型的**最后⼀个全连接层**.

通过简单的**参数配置来指定GLUE中存在任务类型**(如: CoLA对应⽂本⼆分类, MRPC对应句⼦对⽂本⼆分类, STS-B对应句⼦对⽂本多分类), 以及**指定需要微调的预训练模型**.
